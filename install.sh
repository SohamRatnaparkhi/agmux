#!/usr/bin/env bash
# amux installer — idempotent. Symlinks configs to this repo so `git pull` updates everything.
...
link bin/amux         "$HOME/.local/bin/amux"
chmod +x "$REPO/bin/amux"
...
if [ -d "$DR" ]; then cp "$REPO/dracula/amux_status.sh" "$DR/amux_status.sh" && chmod +x "$DR/amux_status.sh" && echo "copied  $DR/amux_status.sh"
...
if [ ! -f "$HOME/.amux/prefix.conf" ]; then
  mkdir -p "$HOME/.amux"
  KEY="C-a"
  if [ -t 0 ]; then
    echo
  ...
  printf 'unbind C-b\nset -g prefix %s\nbind %s send-prefix\n' "$KEY" "$KEY" > "$HOME/.amux/prefix.conf"
  echo "leader   $KEY  (change later: amux prefix <key>)"
fi

# zsh shortcuts: source the repo file from .zshrc (append once)
ZLINE="source \"$REPO/zsh/amux.zsh\""
