#!/usr/bin/env bash
# ax installer — idempotent. Symlinks configs to this repo so `git pull` updates everything.
...
if [ -d "$DR" ]; then cp "$REPO/dracula/ax_status.sh" "$DR/ax_status.sh" && chmod +x "$DR/ax_status.sh" && echo "copied  $DR/ax_status.sh"
